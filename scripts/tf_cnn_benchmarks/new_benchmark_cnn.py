...
import cnn_util
import constants
import new_data_utils as data_utils
import preprocessing
import datasets
import flags
import new_variable_mgr as variable_mgr
...
def create_config_proto(params):
  ...
  print("intra_threads: {}".format(params.num_intra_threads))
  print("inter_threads: {}".format(params.num_inter_threads))
...
class BenchmarkCNN(object):
  ...
  def __init__(self, params, dataset=None, model=None):
    ...
    self.dataset = data_utils.Dataset(self.params.data_dir,
    ...
    self.use_synthetic_gpu_images = False
    #self.dataset.use_synthetic_gpu_images()
  ...
  def _build_graph(self):
    ...
    dataloader = data_utils.DataLoader(
        ...
        self.cpu_device,
        self.devices, get_data_type(self.params),
        'train',
        self.dataset)
    update_ops = None
  ...
  def _benchmark_graph(self, graph_info,
    ...
    saver = tf.train.Saver(
          savable_variables(), save_relative_paths=True)
  ...
  def _build_model_with_dataset_prefetching(self):
    ...
    for device_num in range(len(self.devices)):
      with tf.variable_scope(
          'tower_%s' % device_num,
          use_resource=self.use_resource_vars), tf.name_scope('tower_%i' % device_num) as name_scope:
      #with self.variable_mgr.create_outer_variable_scope(
      #    device_num), tf.name_scope('tower_%i' % device_num) as name_scope:
        #function_buffering_resource = function_buffering_resources[device_num]
        results = self.add_forward_pass_and_gradients(
  ...
  def allreduce_algorithm(self):
    if self.params.device == 'cpu':
      ...
      gpu_indices = [x for x in range(self.params.num_gpus)]
  ...
  def add_forward_pass_and_gradients(self,
    ...
    with tf.device(self.raw_devices[rel_device_num]):
      ...
      logits, aux_logits = self.model.build_network(
          ...
          self.data_format)
      #, self.params.use_tf_layers, self.params.fp16_vars)
      ...
      params = trainable_variables_on_device(
          rel_device_num)
      fp32_params = params
      ...
      param_refs = trainable_variables_on_device(
          rel_device_num)
  ...
  def get_image_preprocessor(self):
    ...
    processor_class = preprocessing.RecordInputImagePreprocessor
