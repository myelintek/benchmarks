...
from tensorflow.data.experimental import shuffle_and_repeat
...
def parse_example_proto(example_serialized):
  ...
  ## myelintek ##
  # get filename for data manipulation.
  feature_map.update({'image/filename': tf.FixedLenFeature([], dtype=tf.string,
                                                           default_value='')})
  ...
  def build_prefetch_input_processing(self, batch_size, model_input_shapes,
    ...
    with tf.device(cpu_device):
      if doing_eval:
        ...
        subset = 'train'
      if getattr(params, 'filename', None):
          self._get_filename = True
      else:
          self._get_filename = False
...
class AdvPreprocessor(InputPreprocessor):
  def __init__(self,
               batch_size,
               output_shapes,
               num_splits,
    ...
    super(AdvPreprocessor, self).__init__(batch_size, output_shapes)
  ...
  def minibatch(self,
    ...
    with tf.name_scope('batch_processing'):
      if subset == 'train':
        raw_images, raw_labels = self.train_input_fn(params.data_dir, self.batch_size_per_split)
      else:
        raw_images, raw_labels = self.eval_input_fn(params.data_dir, self.batch_size_per_split)
      # Build final results per split.
      images = [[] for _ in range(self.num_splits)]
      labels = [[] for _ in range(self.num_splits)]
      for split_index in xrange(self.num_splits):
        images[split_index] = raw_images
        labels[split_index] = raw_labels
...
class BaseImagePreprocessor(InputPreprocessor):
  ...
  def create_dataset(self,
    ...
    if datasets_use_caching:
      ds = ds.cache()
    if train:
      ds = ds.apply(shuffle_and_repeat(buffer_size=10000))
    else:
      ds = ds.repeat()
    ds = ds.apply(
        map_and_batch(
...
class RecordInputImagePreprocessor(BaseImagePreprocessor):
  ...
  def set_aug_list(self, aug_list):
    for aug_name in aug_list:
      mod_path = "aug.image."+aug_name[:-3]
      mod = importlib.import_module(mod_path)
      self.aug_functions.append(mod.run)
  ...
  def minibatch(self,
    ...
    with tf.name_scope('batch_processing'):
      ...
      labels = [[] for _ in range(self.num_splits)]
      ## myelintek ##
      filenames = [[] for _ in range(self.num_splits)]
      if params.use_datasets:
        ...
        for d in xrange(self.num_splits):
          ## myelintek, get filename ##
          if self._get_filename:
              images[d], labels[d], filenames[d] = ds_iterator.get_next()
          else:
      ...
      # always use datasets.
        ...
        for idx in xrange(self.batch_size):
          ...
          ## myelintek, get filename ##
          if self._get_filename:
              (image, label, filename) = self.parse_and_preprocess(value, idx)
              filenames[split_index].append(filename)
          else:
      ...
      for split_index in xrange(self.num_splits):
        ...
        if self._get_filename:
            ## myelintek, get filename ##
            filenames[split_index] = tf.reshape(filenames[split_index],
                                                [self.batch_size_per_split])
      ## myelintek, get filename ##
      if self._get_filename:
...
class LibrispeechPreprocessor(InputPreprocessor):
  ...
  def create_dataset(self,
    ...
    # latter uses map_and_batch(). Try to merge them.
    assert self.supports_datasets()
    glob_pattern = dataset.tf_record_pattern(subset)
    file_names = gfile.Glob(glob_pattern)
    if not file_names:
