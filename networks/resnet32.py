...
class ResnetModel(model_lib.CNNModel):
  ...
  def __init__(self, model, layer_counts, params=None):
    ...
    super(ResnetModel, self).__init__(model, model.MODEL_TYPE_IMAGE_CLASSIFICATION, 224, batch_size, .128,
...
class UserModel(model_lib.CNNModel):
  ...
  def __init__(self, model='Resnet32_v2', layer_counts=(5, 5, 5), params=None):
    if 'v2' in model:
      self.version = 'v2'
    else:
      self.version = 'v1'
    super(UserModel, self).__init__(
        model, model_lib.MODEL_TYPE_IMAGE_CLASSIFICATION, 32, 128, 0.1, layer_counts, params=params)
